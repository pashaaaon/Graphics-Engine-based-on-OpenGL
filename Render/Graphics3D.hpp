...
class EAPI_Model_3D {
    public:
        ...
        bool load(const char *file_dir) {
            ...
            // Texture available
            TextureBool = model.TEXTURES;
            
            // Materials load
            for (SYSTEM_MATERIAL &material : model.MATERIALS) {Materials.push_back(material);}
            glfwPollEvents();
            ...
            for (vector<float> &polygon_group : model.POLYGON_GROUPS) {
                ...
                VAOs.push_back(VAO);
                VBOs.push_back(VBO);
                VBOs_Size.push_back(polygon_group.size());
            }
            glfwPollEvents();
...
class EAPI_Scene_3D {
    public:
        ...
        EAPI_Scene_3D() {
            ...
            glBindBuffer(GL_TEXTURE_BUFFER, light_buffer);
